breed [reactants1 reactant1]     ;; reactant molecules (2H2), shown as white
breed [reactants2 reactant2]     ;; reactant molecules (O2), shown as green
breed [products product]       ;; product molecules (2H2O), shown as red
...
to setup
  clear-all
  set-default-shape products "2H2O"
  set-default-shape reactants1 "2H2"
  set-default-shape reactants2 "O2"
  draw-box                                 ;; draws the reaction container
  create-reactants1 reactant1-amount                    ;; generates reactants
      [move-to one-of black-patches ]       ;; distribute reactants around the world
  create-reactants2 reactant2-amount
  [set color green
    move-to one-of black-patches ]
  create-products product-amount
  [set color red
    move-to one-of black-patches]
...
to update-concentrations
  set react-conc (count reactants1 / count black-patches) * 100
...
to go
    ...
    ;; temperature of the reaction.
     set Kf (5 + (75000 /(8.314 * (temp + 273))))
     set Kr (15 + (18000 / (8.314 * (temp + 273))))
  ask turtles
...
to react-forward  ;; turtle procedure
  let chance Kf
  if (any? reactants2-here) and (random-float 10.0 < chance)
    [ ask one-of other reactants2-here
        [ die ]
      set breed products
      set color red
    ]
...
to react-backward  ;; turtle procedure
  ...
  if random-float 1000.0 < chance
    [ set breed reactants1
      hatch 1
        [ rt 180 ] ]
  if random-float 1000.0 < chance
    [ set breed reactants2
      set color green
      hatch 1
        [ rt 180 ] ]
  [die]
...
true
0
Circle -1 true false 97 78 64
Circle -1 true false 156 171 63
Circle -1 true false 97 168 64
Circle -1 true false 157 78 64

2h2o
true
0
Circle -7500403 true true 44 29 91
Circle -1 true false 194 104 63
Circle -1 true false 16 105 58
Circle -1 true false 119 164 63
Circle -7500403 true true 179 164 91
Circle -1 true false 136 45 58
...
true
0
Circle -7500403 true true 59 119 91
Circle -7500403 true true 149 119 91
